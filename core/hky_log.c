...
#if (HKY_DEBUG)
static void hky_log_memory_writer(hky_log_t *log,hky_uint_t level,hky_uchar *buf,size_t len);
...
static hky_command_t hky_errlog_commands[]={

};

static hky_core_module_t hky_errlog_module_ctx={

};

hky_module_t hky_errlog_module={
...
void
hky_log_error_core(hky_uint_t level,hky_log_t *log,hky_err_t err,const char*fmt,...)
#else
void
hky_log_error_core(hky_uint_t level,hky_log_t *log,hky_err_t err,const char*fmt,va_list args)
#endif
{
#if(HKY_HAVE_VARIADIC_MACROS)
    va_list args;
#endif
    hky_uchar *p,*last,*msg;
    ssize_t n;
    hky_uint_t wrote_stderr,debug_connection;
    hky_uchar errstr[HKY_MAX_ERROR_STR];
    ...
    p=hky_slprintf(p,last,"%P#" HKY_TID_T_FMT ": ",hky_log_pid,hky_log_tid);
    ...
    if(!hky_use_stderr||level>HKY_LOG_WARN||wrote_stderr){
        return;
    }
...
#if !(HKY_HAVE_VARIADIC_MACROS)
void hky_cdecl
hky_log_error(hky_uint_t level,hky_log_t *log,hky_err_t err,const char *fmt,...){
    va_list args;
...
hky_log_abort(hky_err_t err,const char* fmt,...){
    hky_uchar *p;
    va_list args;
    hky_uchar errstr[HKY_MAX_CONF_ERRSTR];

    va_start(args,fmt);
    p=hky_vsnprintf(errstr,sizeof(errstr)-1,fmt,args);
    va_end(args);

    hky_log_error(HKY_LOG_ALERT,hky_cycle->log,err,"%*s",p-errstr,errstr);
}
void hky_cdecl
...
hky_uchar *
hky_log_errno(hky_uchar *buf,hky_uchar *last,hky_err_t err){
    if(buf>last-50){
        buf=last-50;
        *buf++='.';
        *buf++='.';
    ...
    #if(HKY_WIN32)
        buf=hky_slprintf(buf,last,((unsigned)err<0x80000000)? " (%d: " : " (%Xd: ".err);
    #else
...
hky_log_init(hky_uchar *prefix){
    ...
    name=(hky_uchar*) HKY_ERROR_LOG_PATH;
    ...
    if(prefix){
        ...
        if(plen){
            ...
            hky_cpystrn(p,(hky_uchar*)HKY_ERROR_LOG_PATH,nlen+1);
            p=name;
        }
    }
    ...
    hky_log_file.fd=hky_open_file(name,HKY_FILE_APPEND,
        ...
        if(hky_log_file.fd==HKY_INVALID_FILE){
            hky_log_stderr(hky_errno,"[alert] could not open error log file: "
...
hky_int_t
hky_log_open_default(hky_cycle_t *cycle){
    hky_log_t *log;
...
hky_int_t
hky_log_redirect_stderr(hky_cycle_t *cycle){
    hky_fd_t fd;
...
hky_log_set_levels(hky_conf_t *cf,hky_log_t *log){
    ...
    for(i=2;i<cf->args->nelts;i++){
        ...
        if(!found){
            hky_conf_log_error(HKY_LOG_EMERG,cf,0,"invalid log level  \"%V\"",
                                                            &value[i]);
            return HKY_CONF_ERROR;
        }
    }
...
hky_log_set_log(hky_conf_t *cf,hky_log_t **head){
    ...
    if(hky_strcmp(value[1].data,"stderr")==0){
        ...
        if(size==(size_t)HKY_ERROR||size<needed){
            hky_conf_log_error(HKY_LOG_EMERG,cf,0,"invalid buffer size \"%V\" ",&value[1]);
            return HKY_CONF_ERROR;
        }
        ...
        buf->pos=hky_slprintf(buf-start,buf->end,"MEMLOG %uz %V:%ui%N",
         ...
         new_log->writer=buf;
        #else
        hky_conf_log_error(HKY_LOG_EMERG,cf,0,"husky was built without debug support");
        return HKY_CONF_ERROR;
        #endif
...
#if(HKY_DEBUG)
static void
hky_log_memory_writer(hky_log_t *log,hky_uint_t level,hky_uchar *buf,size_t len){
    hky_uchar *p;
