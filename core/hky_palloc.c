...
static hky_inline void *hky_palloc_small(hky_pool_t *pool,size_t size,hky_uint_t align);
...
hky_create_pool(size_t size,hky_log_t *log){
    ...
    p->d.last=(hky_uchar*)p+sizeof(hky_pool_t);
    p->d.end=(hky_uchar*)p+size;
    p->d.next=NULL;
    p->d.failed=0;
...
hky_palloc_block(hky_pool_t *pool,size_t size){
    ...
    psize=(size_t)(pool->d.end-(hky_uchar*)pool);
...
hky_pool_cleanup_file(void *data){
    ...
    hky_log_debug1(HKY_LOG_DEBUG_ALLOC,c->log,0,"file cleanup: fd:%d",c->fd);

    if(hky_close_file(c->fd)==HKY_FILE_ERROR){
        hky_log_error(HKY_LOG_ALERT,c->log,hky_errno,hky_close_file_n " \"%s\" failed",c->name);
    }
}
void
